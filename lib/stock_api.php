...
function fetch_quote(string $symbol, array &$errors): ?array
{
    ...
    if (!$result["ok"]) {
        error_log("fetch_quote failed: " . json_encode($result));
        return null;
    }
    $json_key = "Global Quote";
    $decoded = decode_api_response($result, $json_key, $errors);
    $quote_json = $decoded[$json_key] ?? [];
    if (!is_array($quote_json)) {
        error_log("fetch_quote received an unexpected response shape");
        return null;
    }
...
function search_companies(string $keywords, array &$errors): array
{
    ...
    if (!$result["ok"]) {
        error_log("search_companies failed: " . json_encode($result));
        return [];
    }
    $json_key = "bestMatches";
    $decoded = decode_api_response($result, $json_key, $errors);
    $matches_json = $decoded[$json_key] ?? [];
    if (!is_array($matches_json)) {
        error_log("search_companies received an unexpected response shape");
        return [];
    }
    ...
    foreach ($matches_json as $match_json) {
        ...
        if (empty($company["symbol"]) || strlen($company["symbol"]) > 6) {
            continue;
        }
